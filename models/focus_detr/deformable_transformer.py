...
from .ms_deform_attn import MSDeformAttn, MSDeformAttnDec
...
class DeformableTransformer(nn.Cell):
    def __init__(
        ...
        if num_feature_levels > 1:
            if self.num_encoder_layers > 0:
                self.level_embed = Parameter(
                    Tensor(np.zeros((num_feature_levels, d_model)), mindspore.float32),
                    name="level_embed",
                    requires_grad=True,
                )
            else:
        ...
        if two_stage_type == "standard":
            ...
            if two_stage_pat_embed > 0:
                self.pat_embed_for_2stage = Parameter(
                    Tensor(np.zeros((two_stage_pat_embed, d_model)), mindspore.float32),
        ...
        self.alpha = Parameter(Tensor(np.zeros(3), mindspore.float32), name="alpha", requires_grad=True)
...
class TransformerEncoder(nn.Cell):
    ...
    def get_reference_points(spatial_shapes, valid_ratios):
        ...
        for lvl, (H_, W_) in enumerate(spatial_shapes):
            start = Tensor(0.5, mindspore.float32)
            end = Tensor(H_ - 0.5, mindspore.float32)
            h = int(H_.asnumpy())
            H_m = ops.linspace(start, end, h)
            start = Tensor(0.5, mindspore.float32)
            end = Tensor(W_ - 0.5, mindspore.float32)
            w = int(W_.asnumpy())
            W_m = ops.linspace(start, end, w)
    ...
    def construct(
        ...
        if ref_token_index is not None:
            input_index = mindspore.numpy.tile(expand_dims(ref_token_index, -1), [1, 1, self.d_model])
        ...
        for layer_id, layer in enumerate(self.layers):
            tgt_index = mindspore.numpy.tile(expand_dims(topk_inds[layer_id], -1), [1, 1, output.shape[-1]])
            tgt = ops.gather_elements(output, dim=1, index=tgt_index)
            pos_index = mindspore.numpy.tile(expand_dims(topk_inds[layer_id], -1), [1, 1, output.shape[-1]])
            pos = ops.gather_elements(ori_pos, dim=1, index=pos_index)
            res_index = mindspore.numpy.tile(expand_dims(topk_inds[layer_id], -1), [1, 1, S_ * P_])
            ...
            if sparse_token_nums is None:
                output = ops.tensor_scatter_elements(
                    output, mindspore.numpy.tile(ops.expand_dims(topk_inds[layer_id], -1), [1, 1, tgt.size(-1)]), tgt
                )
            else:
                outputs = []
                ...
                for i in range(topk_inds[layer_id].shape[0]):
                    outputs.append(
                        ops.tensor_scatter_elements(
                            output[i],
                            mindspore.numpy.tile(expand_dims(topk_inds[layer_id][i], -1), [1, tgt.shape[-1]]),
